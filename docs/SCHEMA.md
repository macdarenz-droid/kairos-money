# Current data model

Schema version: 8. Generated by `npm run schema` from canonical SQL migrations.

Money columns are signed INTEGER minor units, restricted to the JavaScript/native bridge exact integer range ±9,007,199,254,740,991. All arithmetic occurs in bigint. Positive is credit; negative is debit, including credit-account purchases. Confidence uses integer basis points (0–10000). JSON money is decimal integer strings when nested in JSON fields. Dates use ISO text; import normalization validates calendar dates against the statement period.

Drizzle models typed queries. SQL migrations enforce the complete constraints and foreign keys. Baseline rollback is destructive and explicitly guarded; subsequent migrations are reversible without deleting records. Coverage rows retain source provenance; union/gap functions exclude uncovered days from averages. transaction_sources retains every contributing batch, so shared transactions can survive a batch rollback.

## account_order

| Column | Storage | Required | Default |
|---|---|---|---|
| account_id | TEXT | Yes | — |
| position | INTEGER | Yes | — |

References: account_id → accounts.id (NO ACTION).

## accounts

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| name | TEXT | Yes | — |
| institution | TEXT | Yes | '' |
| type | TEXT | Yes | — |
| currency | TEXT | Yes | — |
| mask_last4 | TEXT | No | — |
| opening_balance_minor | INTEGER | Yes | 0 |
| archived_at | TEXT | No | — |

## app_settings

| Column | Storage | Required | Default |
|---|---|---|---|
| key | TEXT | Yes | — |
| value | TEXT | Yes | — |

## categories

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| parent_id | TEXT | No | — |
| name | TEXT | Yes | — |
| kind | TEXT | Yes | — |

References: parent_id → categories.id (NO ACTION).

## coverage_ranges

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| account_id | TEXT | Yes | — |
| period_start | TEXT | Yes | — |
| period_end | TEXT | Yes | — |
| import_batch_id | TEXT | Yes | — |

References: import_batch_id → import_batches.id (CASCADE); account_id → accounts.id (NO ACTION).

## debts

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | No | — |
| name | TEXT | Yes | — |
| account_id | TEXT | No | — |
| currency | TEXT | Yes | — |
| balance_minor | INTEGER | Yes | — |
| annual_rate_bp | INTEGER | Yes | — |
| minimum_minor | INTEGER | Yes | — |
| due_day | INTEGER | No | — |
| opened_at | TEXT | Yes | — |
| closed_at | TEXT | No | — |

References: account_id → accounts.id (SET NULL).

## fx_rates

| Column | Storage | Required | Default |
|---|---|---|---|
| as_of | TEXT | Yes | — |
| base | TEXT | Yes | — |
| quote | TEXT | Yes | — |
| rate_e8 | INTEGER | Yes | — |
| source | TEXT | Yes | — |
| fetched_at | TEXT | Yes | — |

## goals

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| name | TEXT | Yes | — |
| target_minor | INTEGER | Yes | — |
| target_date | TEXT | No | — |
| funded_minor | INTEGER | Yes | 0 |
| kind | TEXT | Yes | — |
| currency | TEXT | Yes | — |

## import_batches

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| account_id | TEXT | No | — |
| source_file_hash | TEXT | Yes | — |
| file_name | TEXT | Yes | — |
| issuer_id | TEXT | No | — |
| parser_version | TEXT | Yes | — |
| period_start | TEXT | No | — |
| period_end | TEXT | No | — |
| status | TEXT | Yes | — |
| stated_opening_minor | INTEGER | No | — |
| stated_closing_minor | INTEGER | No | — |
| created_at | TEXT | Yes | — |
| integrity_tier | TEXT | No | — |
| source_rank | INTEGER | Yes | 0 |

References: account_id → accounts.id (NO ACTION).

## insights

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| created_at | TEXT | Yes | — |
| kind | TEXT | Yes | — |
| severity | TEXT | Yes | — |
| title | TEXT | Yes | — |
| body | TEXT | Yes | — |
| evidence | TEXT | Yes | — |
| state | TEXT | Yes | — |
| projected_effect_minor | INTEGER | Yes | — |
| currency | TEXT | Yes | — |
| research_id | TEXT | Yes | — |
| action | TEXT | Yes | — |
| threshold | TEXT | Yes | — |

## ious

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | No | — |
| person | TEXT | Yes | — |
| direction | TEXT | Yes | — |
| currency | TEXT | Yes | — |
| amount_minor | INTEGER | Yes | — |
| reason | TEXT | Yes | — |
| occurred_on | TEXT | Yes | — |
| transaction_id | TEXT | No | — |
| settled_at | TEXT | No | — |

References: transaction_id → transactions.id (SET NULL).

## merchants

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| canonical_name | TEXT | Yes | — |
| aliases | TEXT | Yes | '[]' |
| default_category_id | TEXT | No | — |
| mcc | TEXT | No | — |

References: default_category_id → categories.id (NO ACTION).

## payslips

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| employer | TEXT | Yes | — |
| pay_date | TEXT | Yes | — |
| period_start | TEXT | Yes | — |
| period_end | TEXT | Yes | — |
| gross_minor | INTEGER | Yes | — |
| net_minor | INTEGER | Yes | — |
| tax_minor | INTEGER | Yes | — |
| super_minor | INTEGER | Yes | — |
| deductions | TEXT | Yes | '[]' |
| allowances | TEXT | Yes | '[]' |
| ytd | TEXT | Yes | '{}' |
| currency | TEXT | Yes | — |
| linked_transaction_id | TEXT | No | — |
| import_batch_id | TEXT | Yes | — |

References: import_batch_id → import_batches.id (NO ACTION); linked_transaction_id → transactions.id (NO ACTION).

## privacy_log

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| created_at | TEXT | Yes | — |
| action | TEXT | Yes | — |
| import_batch_id | TEXT | No | — |
| metadata | TEXT | Yes | — |

References: import_batch_id → import_batches.id (NO ACTION).

## profiles

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| period | TEXT | Yes | — |
| archetype | TEXT | No | — |
| axis_scores | TEXT | Yes | — |
| confidence | INTEGER | Yes | — |
| version | INTEGER | Yes | — |
| covered_days | INTEGER | Yes | — |

## rules

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| priority | INTEGER | Yes | — |
| matcher | TEXT | Yes | — |
| action | TEXT | Yes | — |
| created_by | TEXT | Yes | — |

## signals

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| period | TEXT | Yes | — |
| key | TEXT | Yes | — |
| value | TEXT | No | — |
| computed_at | TEXT | Yes | — |
| version | INTEGER | Yes | — |
| status | TEXT | Yes | — |
| inputs | TEXT | Yes | — |

## staging_rows

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| import_batch_id | TEXT | Yes | — |
| source_row_id | TEXT | Yes | — |
| payload | TEXT | Yes | — |
| confidence | INTEGER | Yes | — |
| issues | TEXT | Yes | '[]' |

References: import_batch_id → import_batches.id (CASCADE).

## transaction_sources

| Column | Storage | Required | Default |
|---|---|---|---|
| transaction_id | TEXT | Yes | — |
| import_batch_id | TEXT | Yes | — |
| source_row_id | TEXT | Yes | — |
| original_payload | TEXT | Yes | — |

References: import_batch_id → import_batches.id (CASCADE); transaction_id → transactions.id (CASCADE).

## transactions

| Column | Storage | Required | Default |
|---|---|---|---|
| id | TEXT | Yes | — |
| account_id | TEXT | Yes | — |
| posted_date | TEXT | Yes | — |
| value_date | TEXT | No | — |
| amount_minor | INTEGER | Yes | — |
| currency | TEXT | Yes | — |
| raw_description | TEXT | Yes | — |
| merchant_id | TEXT | No | — |
| category_id | TEXT | No | — |
| subcategory_id | TEXT | No | — |
| type | TEXT | Yes | — |
| transfer_group_id | TEXT | No | — |
| is_recurring | INTEGER | Yes | 0 |
| fingerprint | TEXT | Yes | — |
| import_batch_id | TEXT | No | — |
| confidence | INTEGER | Yes | — |
| user_verified | INTEGER | Yes | 0 |
| notes | TEXT | Yes | '' |
| fx_numerator | TEXT | No | — |
| fx_denominator | TEXT | No | — |
| fx_quote_currency | TEXT | No | — |
| status | TEXT | Yes | 'settled' |

References: import_batch_id → import_batches.id (NO ACTION); subcategory_id → categories.id (NO ACTION); category_id → categories.id (NO ACTION); merchant_id → merchants.id (NO ACTION); account_id → accounts.id (NO ACTION).

## Import staging payloads

Reserved source row `__file__` contains a user-selected file as base64 in encrypted SQLite until extraction or discard. Reserved `__document__` contains the complete normalized document, account context, stated balances and optional payslip. Ordinary source rows contain editable normalized transactions and their issues. Committed documents remain available for deterministic rollback; these are not duplicate live ledger entries. Corrections can carry deferred merchant rules applied only at commit.

## Export-first metadata

Storage schema 4 adds fx_rates: published exchange rates keyed by the day they belong to, so a transaction is valued at the rate in force when it happened rather than at the rate today. Rates are integers scaled by 1e8; they are ratios, not money, and never enter minor-unit arithmetic. Storage schema 3 adds transaction status and batch integrity/source rank. Document payloads retain sessionId, sourceKind, sourceRank, integrityTier and optional running balances. App settings contain confirmed export mappings and the optional reminder weekday. privacy_log retains before/after supersession evidence; transaction_sources keeps original inputs. Export schema_version 3 adds fx_rates to export and backup (optional in older exports), with database_schema_version identifying migrated storage. app_settings keys starting secret: are never exported, backed up or restored (ADR 0044). app_settings keys ai-category:<merchant key> hold the Claude category for a merchant ({category, confidence, model, at}) and ai-run:<id> the previous values that Undo restores (ADR 0047). privacy_log action advisor_call records each optional advisor call: model, token counts, cost in micro-dollars and result.

## Intelligence snapshots

Version 1 uses existing tables without changing storage version. Signals use currency-prefixed period IDs; inputs retain the versioned signal, its window, coverage, pays and covered-day count, and cite the transactions they were computed from by id in evidence rather than copying them; the ledger and transaction_sources remain the single copy of provenance. Profiles store monthly axes, covered days and evidence-completeness confidence. Insights retain the five-part contract and conditional calculation in evidence JSON. app_settings keys intelligence:dismissals, intelligence:metadata, intelligence:reflection and intelligence:buffer:<currency> retain dismissal counts, explicitly supplied purchase context, optional non-validated reflections and the chosen reserve. Goals are user earmarks, never additional ledger balances. All are encrypted and included in export/delete.

## Manual source records

Manual entries use explicit manual-entry-v1 source envelopes in import_batches and a reserved __manual__ staging payload. They have no statement period, no source file and no coverage ranges. The payload stores positive integer-string amount, expense/income/transfer kind, account IDs, date, description, category, notes and confirmed source links. transactions and transaction_sources contain the live projection; transfers project two equal/opposite legs atomically. A confirmed import match suppresses only the corresponding manual projection, retaining the original record for rollback. Export, encrypted backup and deletion include these envelopes using the existing schema. See ADR/0017-manual-source-records.md.

## Transaction attachments

Encrypted app_settings keys ledger-detail:<transaction-id> and ledger-detail:manual:<manual-id> retain a note and receipt records (id, name, base64 data and locally extracted text). Receipts never create ledger transactions or coverage. Existing export/backup/delete include these values; manual deletion removes its attachment key. Import rollback retains attachment metadata for source reimport. See ADR/0018-session4-visuals-and-responsive-ledger.md.

## Recorded valuations, account ownership and notification preferences

Encrypted app_settings net-worth:<valuation-id> values contain id, itemId, name, asset/liability kind, currency, ISO valuation date, positive integer-string minor units and an optional represented accountId. Repeated values update the same holding over time without changing transactions or coverage. Encrypted net-worth-account:<account-id> values contain an explicit include or exclude choice. Included balances use only the latest committed Tier A import with a stated closing balance; manual account links and imported inclusion are mutually exclusive. Notification preferences are stored under notifications:preferences, with four independent boolean opt-ins. Existing full export, backup and delete include all settings. Native notification alarms contain only generic kinds, event hashes and times, never financial amounts or descriptions.

## User category overrides

Encrypted app_settings category-edit:<canonical-transaction-id> stores the selected category or explicit null. Import rebuilds replay it only for an existing non-transfer transaction; original statements, amounts and source provenance remain unchanged. Metadata survives source rollback and backup for later reimport. Bulk edits are transactional. See ADR/0021-user-category-overrides.md.

## Cancellation progress

Encrypted app_settings cancellation:<JSON currency/merchant pair> stores normalized merchant, currency, requested/provider-confirmed status, contact/confirmation date and note. Entries are included in full export, backup, restore and deletion. They do not alter transactions, coverage, observed bills or forecast amounts. Records remain visible without an active detected pattern; later covered settled debits link back to statement evidence. See ADR/0024-cancellation-records.md.

## Confirmed refund links

Encrypted app_settings refund:<credit-id> holds the original purchase id and both source amounts, currencies, account ids and dates. Only settled imported non-transfer credits and earlier same-currency purchases are eligible. Whole credits link once; multiple partial refunds cannot exceed the original purchase. Payslip-linked salary is excluded. Changed, missing, malformed or over-refunded sources make links inactive. Snapshot classification is derived as refund without writing ledger categories; original cash movements remain intact. Purchase-cohort net totals may include later refunds through the analysis date and are distinct from period cashflow. Backup and exact rollback/reimport preserve metadata. See ADR/0029-confirmed-refund-links.md.

## Recorded original currency

Encrypted app_settings foreign-amount:<transaction-id> holds posted amount/currency, original positive amount/currency and a required source note. Reads are active only while the settled transaction matches the saved posted values. Exact rational implied rates are derived in major currency units, with a rounded six-decimal display; no live quote, fee inference or ledger conversion. Settings survive source rollback/reimport and encrypted backup. Transaction FX columns remain reserved and unchanged, preventing reconciliation from silently dropping user notes. See ADR/0028-recorded-original-currency.md.

## Expense category splits

Encrypted app_settings split:<canonical-transaction-id> retains the source id, currency, signed original minor amount and 2-10 positive category allocations (category, essential/discretionary kind, integer-string minor units). Allocations must conserve the complete expense amount. Snapshot readers apply only allocations that still match a settled non-transfer payment. Statement transactions, source rows and coverage are not modified. Rollback retains metadata for exact reimport; backup/restore/delete include it. Manual expenses use their stable projected transaction id. Explicit statement matching copies valid allocations atomically and rejects conflicting splits; the manual copy remains available on source rollback, and deleting the manual record removes only that copy. See ADR/0026-expense-category-splits.md and ADR/0027-manual-split-ownership.md.

## Migration metadata

`_migrations(version INTEGER PRIMARY KEY)` is maintained transactionally by the migration runner. It is not financial data and is removed with the database during deletion.
