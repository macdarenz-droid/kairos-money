...
- Batch completed Session 4 implementation into coherent candidates. While CI runs, continue independent implementation, review and documentation against the full milestone scope; do not stop merely because CI is active. Keep the tested candidate SHA separate from newer local work. Diagnose failures from their actual logs and incorporate repairs into the next consolidated candidate, preserving acceptance assertions. New user requests augment the milestone rather than displacing its original requirements. Do not create component deliveries or routine continuation requests.
...
## Accepted follow-on: offline Money Analysis

The user explicitly ordered completion of Session 4 before the proposed Money Analysis session. Keep all 36 discussed capabilities in one subsequent milestone: combined ledger, reconciliation, cashflow, transfers, credit types, salary patterns, categories, merchants, repetition, small payments, frequency/size, anomalies, period comparisons, regular/occasional costs, payday/day-of-week/clusters, recurrence/price changes, BNPL/remittances/FX/fees/refunds/cash, balances/low-balance/account use/surplus, evidence-linked reports/context questions, budgets/what-ifs/conditional forecasts, exports and changes after new imports. Use local calculations and explanatory templates; disclose uncertainty, missing coverage and forecast assumptions. No server or API dependency. This is planned only; it does not replace outstanding Session 4 requirements or authorize notification capture.

The follow-on coaching presentation is declarative: short observations, a compact chart or number, and occasional evidence-based suggestions. No questions or Try it / Adjust / Not now buttons, and no acceptance or experiment workflow. Explain frequency versus price, accumulation, usual ranges, pace and occasional expenses. Current-week observations require current coverage. This presentation preference applies to the planned analysis coaching, not required transaction editing or import review controls.

The architecture for this milestone, including the 36-capability enumeration, the declarative observation contract, the shared single-pass index and the phase sequencing, is in [SESSION_5_ARCHITECTURE.md](SESSION_5_ARCHITECTURE.md). It is a plan only and does not start the work.

The user also accepted the six research-informed ideas for the later Money Analysis session: make patterns understandable, connect them to a meaningful goal and realistic obstacle, explain practical alternatives, translate changes into affordable contributions, show actual progress separately from scenarios, and avoid shame. Present these as quiet statements and compact concepts/visuals, including clearly conditional arithmetic such as two fewer $15 purchases freeing $30 per week. No claim that a statement proves motive or that a hypothetical reduction was saved. Finish Session 4 before implementing this layer.
