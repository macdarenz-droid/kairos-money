...
| 37 | Measured tap and typing baseline reported | **OPEN** | The figures are measured and asserted on the device, but on a green run they reached only `docs/evidence/usability-baseline.json` inside the artifact, which this environment cannot download. `625f656` sends the same report through instrumentation status so it prints in the job log; the numbers are recorded in `docs/LOW_EFFORT_USABILITY.md` once that run reports them. **No tap-count comparison is stated until then.** |
