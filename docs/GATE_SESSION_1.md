...
|---|---|---|
| APK installs and launches | FAIL | Both APKs installed on the CI Android 34 emulator in run 34744181772. The emulator booted in 65.9 seconds, but the target process crashed during the app-flow test. The diagnostic run identified a Google Play Services font-provider dependency kill. The unnecessary transitive emoji initializer is removed, pending rerun; see ci-third-run.json and ci-fourth-run.json. |
| Both themes switch instantly with no wrong-theme flash | FAIL | UI interaction tests verify instant theme state changes in both directions. Pre-render bootstrap, native launch colours and appearance persistence are implemented. Native theme state and resume checks pass, but run 34745565390 screenshots are obscured by a System UI ANR and are rejected. Clean foreground screenshots remain required. |
...
| New GitHub repository and signed debug artifact on push | FAIL | Workflow is written. The user-created private repository is accessible with write permission. Initial source upload 7dc9ae9 and CI source/build/signature checks passed. Runner cleanup resolved the disk blocker on the third run: preflight passed with 28,127,150,080 bytes free and the emulator booted. Native instrumentation then reported a process crash, so successful native workflow/APK publication remains required. See ci-third-run.json and ci-fourth-run.json. Diagnostic capture identified the font-provider dependency; the manifest repair now awaits the native gate. |
