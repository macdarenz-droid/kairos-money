...
vi.mock('@capacitor/core', () => ({Capacitor: {isNativePlatform: () => true}, registerPlugin: (name: string) => name === 'KairosNotices' ? {
  installed: async () => ({apps: [{id: 'com.synthetic.bank', label: 'Synthetic Bank App'}]}),
...
it('every notice in the sheet says which app sent it', async () => {
  ...
  await waitFor(() => expect(screen.getAllByText('From Synthetic Bank App')).toHaveLength(2));
  expect(screen.getByText('From com.synthetic.files')).toBeTruthy();
});
