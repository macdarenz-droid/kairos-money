...
it.each(['dark','light'])('renders muted nine-day freshness and overlapping dates in one %s sheet',async theme=>{
 ...
 expect(view.container.querySelector('.surface-muted')).toBeTruthy();expect(screen.getByText(/^9d$/)).toBeTruthy();fireEvent.click(screen.getByRole('button',{name:'Bring my statements up to date'}));expect(open).toHaveBeenCalledOnce();cleanup();
