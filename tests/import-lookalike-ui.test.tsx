...
it('describes lookalike rows by the rule that matched them: within three days, not the same day', async () => {
  const csv = 'Date,Description,Amount\n01/01/2026,Synthetic shop,-10.00\n03/01/2026,Synthetic shop,-10.00'; await state.repo!.imports.stageFile('lookalike.csv', btoa(csv), hash(csv));
  ...
  for (const [label, value] of [['Statement start', '2026-01-01'], ['Statement end', '2026-01-31'], ['Stated opening balance', '0'], ['Stated closing balance', '-20.00']]) fireEvent.change(screen.getByLabelText(label!), { target: { value } });
  fireEvent.click(screen.getByRole('button', { name: 'Extract for review' }));
  const note = await screen.findByText(/(looks? like|identical to) another row in this same file/, {}, { timeout: 10000 });
  expect(note.textContent).toContain('within three days');
  expect(note.textContent).not.toContain('same day');
});
