...
it('saves, edits and deletes exact manual income and expense without claiming coverage',async()=>{
 await repo.manual.save(entry);expect(await repo.manual.today(entry.date)).toEqual([{currency:'AUD',income:'0',spending:'1250'}]);expect(await repo.imports.batches()).toHaveLength(0);
