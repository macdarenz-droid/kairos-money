...
describe('an approved notice for a different purchase of the same amount', () => {
  it('is dropped when the statement is already imported (approve after import)', async () => {
  ...
  it('disappears when a later import commits (visible first, then gone)', async () => {
    const {driver, raw, repo} = await ready();
    await repo.notices.approve(bakery);
