...
describe('Opt-in imported-data notices',()=>{
 it('defaults off and suppresses all notices without current coverage',()=>{const s=snapshot();expect(notificationPlan(s,defaultNotices)).toEqual([]);s.coverage[0]!.end='2026-09-13';expect(notificationPlan(s,{bill:true,unusual:true,price:true,digest:true})).toEqual([]);});
