...
it.each(['dark','light'])('keeps opt-ins independent and leaves denied preference off in %s',async theme=>{
 ...
 await waitFor(()=>expect((screen.getAllByRole('button',{name:'Off'})[0] as HTMLButtonElement).disabled).toBe(false));
 expect(state.request).not.toHaveBeenCalled();
 state.request.mockResolvedValue({granted:false});fireEvent.click(screen.getAllByRole('button',{name:'Off'})[0]!);
 ...
 state.request.mockResolvedValue({granted:true});fireEvent.click(screen.getAllByRole('button',{name:'Off'})[0]!);
 ...
 fireEvent.click(screen.getByRole('button',{name:'On'}));await waitFor(async()=>expect((await state.repo!.notifications.preferences()).bill).toBe(false));
