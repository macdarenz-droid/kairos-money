...
raw.exec("INSERT INTO import_batches(id,source_file_hash,file_name,parser_version,status,created_at) VALUES('b','synthetic','Synthetic benchmark.csv','benchmark-only','committed','2026-12-31')");
...
raw.close();return {fixture:'synthetic only',rows:20000,reconciliation_ms:Math.round(reconcileMs),sqlite_snapshot_with_provenance_ms:Math.round(readMs),runtime:process.version,scope:'Local Node SQLite; Android cold start and frame times still require device measurement'};
}
