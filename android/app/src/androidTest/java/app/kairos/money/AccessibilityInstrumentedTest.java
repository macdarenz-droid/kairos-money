...
public class AccessibilityInstrumentedTest {
    ...
    @Test public void majorScreensRemainNamedAndOperableAtTwoHundredPercentText() throws Exception {
        try(ActivityScenario<MainActivity> scenario=ActivityScenario.launch(MainActivity.class)) {
            ...
            try {for(String tab:new String[]{"Today","Ledger","Quick","Insights","You"}){click(tab);awaitJs("document.querySelector('nav [aria-current=page]').textContent.trim()==="+JSONObject.quote(tab));assertEquals("0",js("String(Array.from(document.querySelectorAll('button')).filter(e=>{const r=e.getBoundingClientRect();return r.width&&r.height&&(r.width<44||r.height<44)}).length)"));assertEquals("0",js("String(Array.from(document.querySelectorAll('button,input,select,textarea')).filter(e=>{const r=e.getBoundingClientRect();if(!r.width||!r.height)return false;if(e.matches('button'))return !e.textContent.trim()&&!e.getAttribute('aria-label');return !e.labels?.length&&!e.getAttribute('aria-label')}).length)"));assertEquals("false",js("document.documentElement.scrollWidth>window.innerWidth"));NativeEvidence.capture(activity,"text-200-"+tab.toLowerCase());}}
