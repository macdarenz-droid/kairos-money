...
export class FileSource implements TransactionSource {
 ...
 async fetch(options: SourceOptions, progress?: (message: string) => void): Promise<Document> {
  const e = await extract(this.bytes, this.name, progress);
  ...
  const issuer = issuerAdapter(this.institution);
