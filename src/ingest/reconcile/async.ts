...
export async function reconcileAsync(documents:readonly Document[],keep:ReadonlySet<string>=new Set()):Promise<LedgerRow[]>{
 if(typeof Worker==='undefined'||documents.reduce((n,d)=>n+d.rows.length,0)<=200)return reconcile(documents,keep);
 ...
 try{return await new Promise<LedgerRow[]>((resolve,reject)=>{
  ...
  worker.postMessage({documents,keep:[...keep]});
