...
export type RawRow = { sourceId: string; date: string; description: string; amount: string; direction?: 'debit' | 'credit'; reference?: string; pending?: boolean; confidence: number };
export type NormalizedRow = { sourceId: string; accountId: string; date: string; description: string; merchant: string; minor: string; currency: Currency; reference: string; pending: boolean; confidence: number; fingerprint: string; issues: string[]; category: string | null; verified: boolean; duplicateOf: string | null; occurrence: string; createRule: boolean };
