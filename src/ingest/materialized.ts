...
import {categorize,merchantDefaults,ownerRules,type CategoryRule} from '../ledger/rules';
import {aiCategories} from '../ledger/ai-categories';

type Categorising={rules:CategoryRule[];defaults:Record<string,string>;ai:Record<string,string>;edited:Set<string>};
async function categorising(driver:Driver):Promise<Categorising>{
 const edited=new Set((await driver.query("SELECT key FROM app_settings WHERE key>='category-edit:' AND key<'category-edit;'")).map(r=>String(r.key).slice(14)));
 return {rules:await ownerRules(driver),defaults:await merchantDefaults(driver),ai:await aiCategories(driver),edited};
}
/** The category on screen is Claude's: no owner tag, and Claude is the strongest answer that applies. */
function fromClaude(record:SqlRow,chosen:Source,known:Categorising):boolean{
 const category=record.category===null||record.category===undefined?null:String(record.category);
 if(category===null||known.edited.has(String(record.id))||chosen.ownerCategory||!Object.hasOwn(known.ai,merchantOf(record,chosen)))return false;
 const answer=categorize({merchant:merchantOf(record,chosen),minor:String(record.amount_minor)} as LedgerRow,known.rules,known.defaults,chosen.mcc,known.ai);
 return answer.source==='ai'&&answer.category===category;
}
const merchantOf=(record:SqlRow,chosen:Source)=>record.canonical_name===null||record.canonical_name===undefined?chosen.merchant:String(record.canonical_name);

type Source = {batchId:string;sourceId:string;rank:number;merchant:string;mcc:string|null;reference:string;runningBalance?:string;fingerprint:string;issues:string[];duplicateOf:string|null;occurrence:string;createRule:boolean;categoryFrom?:'suggestion';ownerCategory:boolean;pending:boolean;verified:boolean;confidence:number};
...
export async function materializedLedger(driver:Driver):Promise<LedgerRow[]> {
 ...
 const provenance=await queryPages(driver,`SELECT s.transaction_id,s.import_batch_id,s.source_row_id,b.source_rank,
  ...
  json_extract(s.original_payload,'$.categoryFrom') AS category_from,json_extract(s.original_payload,'$.category') AS source_category,
 ...
 const scoped=await scopedBatches(driver),known=await categorising(driver);
 const sources=new Map<string,Source[]>();
 for(const record of provenance){
  if(!scoped.has(String(record.import_batch_id)))continue;
  collect(sources,record);
 }
 return transactions.filter(record=>scoped.has(String(record.import_batch_id))).map(record=>row(record,sources.get(String(record.id)),known));
}

/** Build one ledger row from its transaction record and the provenance that supports it. */
function row(record:SqlRow,evidence:Source[]|undefined,known:Categorising):LedgerRow {
...
function manualSource(record:SqlRow):Source {
 return {batchId:String(record.import_batch_id),sourceId:String(record.source_row_id),rank:Number(record.source_rank),
  ...
  issues:[],duplicateOf:null,occurrence:'',createRule:false,ownerCategory:true,pending:false,verified:true,confidence:10000};
}
...
export async function materializedPage(driver:Driver,search:string,offset:number,limit:number):Promise<LedgerPage> {
 ...
 const sources=new Map<string,Source[]>(),known=await categorising(driver);
 ...
 return {rows:transactions.map(record=>{
  const built=row(record,sources.get(String(record.id)),known);
