...
export type Kind = 'essential'|'discretionary'|'debt'|'income'|'savings'|'transfer'|'unknown';
export type Transaction = { id:string; accountId:string; date:string; minor:string; currency:Currency; description:string; category:string; kind:Kind; status:'pending'|'settled'; transfer:boolean; recurring:boolean; instrument?:'card'|'bnpl'|'cash'|'transfer'; hour?:number; outsideRoutine?:boolean; planned?:boolean; overdraftFee?:boolean; sources?:{file:string;row:string;raw:string}[] };
