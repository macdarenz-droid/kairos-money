...
export function MoneyVisuals(){
 ...
 const sorted=[...categories].sort((a,b)=>a[1].minor>b[1].minor?-1:1),total=sorted.reduce((n,[,r])=>n+r.minor,0n);let left=0;
 ...
 {total===0n?<p>No covered spending for this month.</p>:<><svg viewBox="0 0 1000 120" role="img" aria-label="Category spending proportions">{sorted.map(([name,row])=>{const unit=Number(displayRatio(row.minor.toString(),total.toString()));const width=unit/1000;const x=left;left+=width;return <rect key={name} x={x} y="0" width={width} height="120" fill="var(--surface-3)" stroke="var(--border-default)"/>;})}</svg>{sorted.map(([name,row])=><Row key={name} trailing={<Button variant="quiet" onClick={()=>show(name,row.ids,'Sum of settled spending in this category on covered days.')}><Amount value={money(row.minor,currency(code))} context={name}/></Button>}>{name}</Row>)}</>}
...
function Cashflow({snapshot,window,show}:{snapshot:Snapshot;window:Window;show:(title:string,ids:string[],text:string)=>void}){
 ...
 return <><svg viewBox="0 0 1000 200" role="img" aria-label="Daily net cashflow, broken at missing coverage"><line x1="0" y1="100" x2="1000" y2="100" stroke="var(--border-default)"/>{segments.map((p,i)=><polyline key={i} points={p.join(' ')} stroke="var(--accent)" fill="none" strokeWidth="2"/>)}</svg><p className="meta">{window.start} to {window.end}. Line breaks are missing coverage; stale days are the unreported live edge.</p><details><summary>Daily amounts and sources</summary>{rows.map(r=><Row key={r.date} trailing={r.net===null?<span>{r.state==='stale'?'Stale':r.state==='future'?'Future':'No coverage'}</span>:<Button variant="quiet" onClick={()=>show(r.date,r.evidence,`Income ${r.income}, spending ${r.spending}, net ${r.net} minor units. Covered settled transactions only.`)}><Amount value={money(BigInt(r.net),snapshot.currency)} context={`net cashflow ${r.date}`}/></Button>}>{r.date}</Row>)}</details></>;
}
