...
export function Refunds({id,credit}:{id:string;credit:boolean}){
 ...
 <p className="meta">A matching amount is not proof of a refund. Confirm only when the statement or receipt identifies the original purchase.</p><label className="input-label">Original purchase<select disabled={busy} value={selected} onChange={e=>setSelected(e.target.value)}><option value="">Choose purchase</option>{rows.slice(0,100).map(p=><option key={p.id} value={p.id}>{p.date} · {p.description} · {p.account} · {format(money(-BigInt(p.minor),currency(p.currency)))}</option>)}</select></label>{rows.length>100&&<p>Showing 100 purchases. Narrow the search to find another.</p>}{!rows.length&&<p>No eligible purchase. Import its statement first or leave this credit unlinked.</p>}<Button variant="primary" disabled={busy||!selected} onClick={()=>void act(()=>session.run(r=>r.refunds.save(id,selected)))}>Confirm refund link</Button><Button variant="quiet" disabled={busy} onClick={()=>setEditing(false)}>Cancel refund linking</Button></>}
 {value?.saved&&<Button variant="danger" disabled={busy} onClick={()=>setRemoving(true)}>Remove refund link</Button>}</>:<>{value?.refunds.length?<>{value.refunds.map(r=><Row key={r.id} trailing={<Amount value={money(BigInt(r.minor),currency(r.currency))} context="linked refund"/>}>{r.description}<p>{r.date} · {r.account}</p></Row>)}<Row trailing={<Amount value={money(BigInt(value.remaining!),currency(value.refunds[0]!.currency))} context="purchase amount after linked refunds"/>}>Purchase amount after linked refunds</Row></>:<p>No confirmed refund linked to this purchase.</p>}</>}
 </>}{removing&&<><p>Remove this link? Both statement transactions remain.</p><Button variant="danger" disabled={busy} onClick={()=>void act(()=>session.run(r=>r.refunds.remove(id)))}>Confirm remove refund link</Button><Button variant="quiet" disabled={busy} onClick={()=>setRemoving(false)}>Keep refund link</Button></>}{error&&<p role="alert">{error}</p>}</section>;
}
