...
export function Refunds({id,credit}:{id:string;credit:boolean}){
 ...
 return <section className="stack"><h3>{credit?'Refund link':'Refunds received'}</h3><p className="meta">Link an actual posted refund to its original purchase. The full credit counts once; the original statement amounts and dates stay intact.</p>
 ...
 {editing&&<>{matches.length>0&&<div className="section-gap"><p className="meta">Eligible purchases, closest evidence first. Selecting one fills the choice below; confirming is still a separate step.</p>{matches.map(m=>{const p=byId.get(m.prefill.purchaseId)!;return <Row key={m.id} trailing={<Button disabled={busy} aria-pressed={selected===p.id} onClick={()=>setSelected(p.id)}
 ...
 <Input label="Find original purchase" value={search} onChange={e=>setSearch(e.target.value)}/><p className="meta">Matching currency and amount are not proof of a refund. Confirm only when the statement or receipt identifies the original purchase. One credit can link to one purchase; several partial refunds may link to the same purchase.</p><label className="input-label">Original purchase<select disabled={busy} value={selected} onChange={e=>setSelected(e.target.value)}><option value="">Choose purchase</option>{rows.slice(0,100).map(p=><option key={p.id} value={p.id}>{p.date} · {p.description} · {p.account} · {format(money(-BigInt(p.minor),currency(p.currency)))}</option>)}</select></label>{rows.length>100&&<p>Showing 100 purchases. Narrow the search to find another.</p>}{!rows.length&&<p>No eligible purchase. Import its statement first or leave this credit unlinked.</p>}<Button disabled={busy||!selected} onClick={()=>void act(()=>session.run(r=>r.refunds.save(id,selected)))}>Confirm refund link</Button><Button disabled={busy} onClick={()=>setEditing(false)}>Cancel refund linking</Button></>}
