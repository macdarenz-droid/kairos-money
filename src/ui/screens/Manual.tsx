...
export function ManualHistory({accounts,today=false}:{accounts:Account[];today?:boolean}){
 ...
 {data.data?.totals.map(t=><div key={t.currency}><Row trailing={<Amount value={money(BigInt(t.spending),currency(t.currency))} context="recorded spending today" hero/>}>Spent today</Row>{BigInt(t.income)>0n&&<Row trailing={<Amount value={money(BigInt(t.income),currency(t.currency))} context="recorded income today"/>}>Received today</Row>}</div>)}{data.data&&!data.data.totals.length&&<p>Nothing recorded today yet.</p>}{data.error&&<p role="alert">Today's entries could not be read.</p>}
 ...
 <details><summary>What is counted here</summary><p className="meta">Transactions you recorded by hand, on today's date. Transfers between your own accounts are left out, because moving money is not spending it.</p></details></section>;
