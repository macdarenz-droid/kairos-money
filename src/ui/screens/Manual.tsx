...
export function ManualHistory({accounts,today=false}:{accounts:Account[];today?:boolean}){
 ...
 {data.data?.totals.map(t=><div key={t.currency}><Row trailing={<Amount value={money(BigInt(t.spending),currency(t.currency))} context="recorded spending today" hero/>}>Spent today</Row>{BigInt(t.income)>0n&&<Row trailing={<Amount value={money(BigInt(t.income),currency(t.currency))} context="recorded income today"/>}>Received today</Row>}
 {BigInt(t.awaitingSpending)>0n&&<Row trailing={<Amount value={money(BigInt(t.awaitingSpending),currency(t.currency))} context="spending awaiting a statement"/>}>Spent, waiting on your statement<p className="meta">Your bank announced it; the statement has not confirmed it yet.</p></Row>}
 {BigInt(t.awaitingIncome)>0n&&<Row trailing={<Amount value={money(BigInt(t.awaitingIncome),currency(t.currency))} context="money in awaiting a statement"/>}>Received, waiting on your statement</Row>}</div>)}{data.data&&!data.data.totals.length&&<p>Nothing recorded today yet.</p>}{data.error&&<p role="alert">Today's entries could not be read.</p>}
 ...
 <details><summary>What is counted here</summary><p className="meta">Everything recorded on today's date — entered by hand, imported, or approved from a bank notification. Anything your statement has not confirmed yet is counted on its own line. Transfers between your own accounts are left out, because moving money is not spending it.</p></details></section>;
