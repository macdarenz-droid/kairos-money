...
export function NetWorth(){
 ...
 const stored=useQuery({queryKey:['fx-rates'],enabled:session.state==='ready',queryFn:()=>session.run(repo=>repo.rates())});
 if(session.state!=='ready')return null;
 const rates:Rate[]=(stored.data??[]).map(r=>({asOf:r.asOf,base:currency(r.base),quote:currency(r.quote),rateE8:BigInt(r.rateE8),source:r.source}));
 const values=q.data?.values??[],accountPositions=q.data?.accounts??[],history=netWorthHistory(values,currency(code),rates),latest=history.at(-1),items=[...new Map(values.map(v=>[v.itemId,v])).values()],position=combinedPosition(values,accountPositions,currency(code),rates);
 ...
 const bare=!values.length&&!owned.length&&!position.accounts.length;
 const hasFigures=values.length>0||position.accounts.length>0||owned.some(a=>a.choice==='include');
 // Other currencies are reviewed under their own display currency, so only these can be acted on here.
 const pending=owned.some(a=>a.choice==='review'||(a.choice==='include'&&!a.verified));
