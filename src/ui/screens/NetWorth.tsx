...
export function NetWorth(){
 ...
 const values=q.data?.values??[],rates=q.data?.rates??[],accountPositions=q.data?.accounts??[],history=netWorthHistory(values,currency(code),rates),latest=history.at(-1),items=[...new Map(values.map(v=>[v.itemId,v])).values()],position=combinedPosition(values,accountPositions,currency(code),rates);
 ...
 {position.unconverted.length>0&&<p className="meta">Leaves out {position.unconverted.join(', ')}: no rate to {code} yet.</p>}
