...
export function TransactionAttachments({target}:{target:string}){
 ...
 async function saveReceipt(data:string,name:string,signal?:AbortSignal){const bytes=decodeFile(data);try{const text=await new FileSource(bytes,name).receiptText(setProgress);if(signal?.aborted)throw new Error('Receipt capture was cancelled.');await session.run(r=>{if(signal?.aborted)throw new Error('Receipt capture was cancelled.');return r.attachments.attach(target,{id:crypto.randomUUID(),name,data,text});});await refresh();}finally{bytes.fill(0);}}
