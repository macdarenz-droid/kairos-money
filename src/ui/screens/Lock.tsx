...
import { LockArrival, Loader } from '../design/Motion';
...
export function LockScreen() {
  ...
  return <main className="lock-screen">{session.state === 'checking' || session.state === 'background' ? <><Brand/><Loader label="Opening secure storage"/></> : session.state === 'recovery-code' ? <><Brand/><h1>Keep your recovery code</h1><div className="stack"><p>This code restores an encrypted backup on a new phone or after a reset. It does not unlock this installation. Keep it separately from your backups.</p><RecoveryCode value={session.recoveryCode}/><label className="check-row"><input type="checkbox" checked={written} onChange={event => setWritten(event.target.checked)}/>I have written this down.</label><Button variant="primary" disabled={!written || busy} onClick={() => void acknowledge()}>Continue to ledger</Button>{error && <p role="alert">{error}</p>}</div></> : session.state === 'error' ? <><Brand/><h1>Storage needs attention</h1><p>{session.error}</p><div className="section-gap"><Button onClick={() => void session.retry()}>Try again</Button></div></> : <>
    <div className="brand"><LockArrival/><span>Kairos</span></div><div className="lock-rise">
    ...
    </form>{!setup && recovery !== 'replace' && <Button variant="quiet" disabled={busy} onClick={() => { setError(''); setRecovery('options'); }}>Forgot PIN?</Button>}</div></>}
