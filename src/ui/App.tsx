...
import { BrainMarker, TodayStart, WeekStrip } from './screens/Today';
...
export default function App() {
  ...
  const startImport = () => { if (!count) { setSheet('account'); return; } setTab('Ledger'); setSheet(null); setImportRequest(n => n + 1); };
  // Nothing to show yet: one card and one next step instead of a long blank screen.
  const startHere = accounts.isSuccess && (!count || (statementData.isSuccess && !statementData.data.some(b => b.status === 'committed') && !manualCount.data));
  ...
  return <div className="app" aria-hidden={session.state === 'background' || undefined} style={session.state === 'background' ? { display: 'none' } : undefined}><header className="brand-bar"><Brand/><div className="privacy-status"><LockKeyhole size={12}/><span>{session.state === 'preview' ? 'Design preview' : 'On this device'}</span></div></header>
    ...
    {tab === 'Today' && <div className="stack today"><BrainMarker/><Unconverted onFix={() => openSettings('currency')}/>{startHere && <TodayStart onStart={startImport}/>}<MoneyBand/><Button variant={startHere ? 'default' : 'primary'} className="add-primary" onClick={()=>openManualSheet()}><Plus size={18}/>Add transaction</Button><SavingsPath/><Surfaces/><WeekStrip/><ManualHistory today/></div>}
    ...
    {toast && <Toast message={toast} onDismiss={dismissToast} saved/>}
  </div>;
}
