...
export default function App() {
  ...
  return <div className="app"><header className="brand-bar"><Brand/><div className="privacy-status"><LockKeyhole size={12}/><span>{session.state === 'preview' ? 'Design preview' : 'On this device'}</span></div></header>
    ...
    {tab === 'Ledger' && <>{session.state === 'ready' && accounts.isPending ? <Skeleton label="Reading accounts"/> : count ? <><div className="list-heading"><h2>Accounts</h2><span className="meta">Opening balances</span></div>{accounts.data?.map(account => <Row key={account.id} trailing={<Amount value={fromDatabase(account.opening_balance_minor, currency(account.currency))} context={`${account.name} opening balance`}/>}><div className="account-summary"><span className="account-symbol"><WalletCards size={18}/></span><div><h3>{account.name}</h3><p>{account.currency}{account.mask_last4 ? ` · ••${account.mask_last4}` : ''}</p></div></div></Row>)}</> : <EmptyState icon={<FileText size={28} strokeWidth={1.3}/>} title="A clean ledger" action={accountAction}>Start with the account your salary arrives in, then import its statements.</EmptyState>}<ImportWorkspace accounts={accounts.data ?? []} request={importRequest} consumed={consumeImport}/></>}
    {tab === 'Insights' && <><EmptyState icon={<Layers3 size={28} strokeWidth={1.3}/>} title="Still learning" action={<Button variant="primary" onClick={() => setTab('Ledger')}>Go to your ledger</Button>}>Your money profile needs at least 60 days of statement coverage. There is no profile or spending advice until your data can support it.</EmptyState><Row trailing={<span className="meta">{days} days</span>}>Verified coverage</Row><Row trailing={<span className="meta">Not available yet</span>}>Money profile</Row><div className="information"><ShieldCheck size={15}/><p>Your future insights will show their evidence. They describe observed spending, without judgement or diagnosis.</p></div></>}
