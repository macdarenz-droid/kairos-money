...
export default function App() {
  ...
  return <div className="app"><header className="brand-bar"><Brand/><div className="privacy-status"><LockKeyhole size={12}/><span>{session.state === 'preview' ? 'Design preview' : 'On this device'}</span></div></header>
    ...
    {tab === 'Today' && <><Freshness accounts={accounts.data??[]} batches={statementData.data??[]} today={localDay()} onUpdate={()=>setSheet('update')}/><EmptyState icon={<WalletCards size={28} strokeWidth={1.3}/>} title="Give your money a starting point" action={count ? <Button onClick={() => setTab('Ledger')}>View your accounts</Button> : accountAction}>Your safe-to-spend amount needs verified statements and upcoming commitments. Until then, there is no estimate to show.</EmptyState><Row trailing={<span className="meta">{count}</span>}>Accounts set up</Row><Row trailing={<span className="meta">{days ? `${days} covered days` : 'No statements yet'}</span>}>Statement coverage</Row><div className="information"><ShieldCheck size={15}/><p>You stay in control. Every statement stays in staging until you confirm its review.</p></div></>}
    ...
    {tab === 'Insights' && <><EmptyState icon={<Layers3 size={28} strokeWidth={1.3}/>} title="Still learning" action={<Button variant="primary" onClick={() => setTab('Ledger')}>Go to your ledger</Button>}>Your money profile needs at least 60 days of statement coverage. There is no profile or spending advice until your data can support it.</EmptyState><Row trailing={<span className="meta">{days} days</span>}>Imported coverage</Row><Row trailing={<span className="meta">Not available yet</span>}>Money profile</Row><div className="information"><ShieldCheck size={15}/><p>Your future insights will show their evidence. They describe observed spending, without judgement or diagnosis.</p></div></>}
