...
function month(s: Snapshot, start: Day, end: Day): MonthFlow {
  const inflow = inflowRows(s, start, end), outflow = spendingRows(s, start, end);
  const received = sum(inflow.map(t => BigInt(t.minor))), spent = out(outflow);
  return {month: start.slice(0, 7), start, end, inMinor: received.toString(), outMinor: spent.toString(), leftMinor: (received - spent).toString(),
    tier: tierFor(s, start, end), evidence: [...inflow, ...outflow].map(t => t.id)};
}
...
export function spending(input: BrainInputs): Spending {
  ...
  const categories: CategoryShare[] = [...byCategory].map(([category, e]) => ({category, minor: e.minor.toString(), share: share(e.minor, spent), evidence: e.ids}))
    .sort((a, b) => BigInt(b.minor) > BigInt(a.minor) ? 1 : BigInt(b.minor) < BigInt(a.minor) ? -1 : a.category.localeCompare(b.category));
  ...
  const weekdays = new Map<Weekday, {minor: bigint; ids: string[]}>();
  for (const t of pattern) {
    const w = ((day(t.date) + 4) % 7) as Weekday, entry = weekdays.get(w) ?? {minor: 0n, ids: []};
    entry.minor += abs(BigInt(t.minor)); entry.ids.push(t.id); weekdays.set(w, entry);
  }
  const busiest = [...weekdays].sort((a, b) => b[1].minor > a[1].minor ? 1 : b[1].minor < a[1].minor ? -1 : a[0] - b[0])[0];
  ...
  if (payDays.size) {
    ...
    if (afterDays > 0n && otherDays > 0n) {
      ...
      paydayEffect = {afterPayDayMinor: a.toString(), otherDayMinor: o.toString(), ratio: share(a, o), evidence: pattern.map(t => t.id)};
    }
  }
  ...
  const days = Array.from({length: 7}, (_, i) => shift(asOf, i - 6)).map(date => {
    const list = spendingRows(s, date, date);
  ...
  return {
    ...
    busiestWeekday: busiest ? {day: busiest[0], minor: busiest[1].minor.toString(), evidence: busiest[1].ids} : null,
