...
/** What a sorting run sends: merchants neither the owner nor a Claude run has sorted; with `onlyNew`, only uncategorised ones. Undo makes a run's merchants eligible again. */
export async function sortingPayload(driver: Driver, rows: readonly LedgerRow[], onlyNew: boolean): Promise<Payload> {
  ...
  const decided = new Set([...rules.map(r => r.merchant), ...Object.keys(defaults), ...Object.keys(await aiCategories(driver))]);
