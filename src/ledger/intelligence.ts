...
export function intelligenceRepository(driver:Driver){
 ...
 async function snapshot(asOf:string,code:string):Promise<Snapshot>{
  ...
  const transactions:Transaction[]=rows.filter(r=>ids.includes(String(r.account_id))).map(r=>({id:String(r.id),accountId:String(r.account_id),date:String(r.posted_date),minor:String(r.amount_minor),currency:currency(String(r.currency)),description:String(r.merchant??r.raw_description),category:String(r.category_name??'Uncategorised'),kind:(r.category_kind??'unknown') as Kind,status:r.status==='pending'?'pending':'settled',transfer:r.transfer_group_id!==null,recurring:r.is_recurring===1,...metadata[String(r.id)]}));
