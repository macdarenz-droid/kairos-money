...
export function netWorthRepository(driver:Driver){
 ...
 async function accountPositions():Promise<AccountPosition[]>{
  const accounts=await driver.query('SELECT id,name,type,currency FROM accounts WHERE archived_at IS NULL ORDER BY name,id'),choices=new Map((await driver.query("SELECT key,value FROM app_settings WHERE key LIKE 'net-worth-account:%'")).map(r=>{let choice:unknown;try{choice=JSON.parse(String(r.value));}catch{choice=null;}return [String(r.key).slice(18),choice];}));
